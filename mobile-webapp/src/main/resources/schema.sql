...
(
   oauth_id             varchar(32) not null  comment '编号',
...
(
   ...
   password             varchar(100) comment '密码(MD5(密码+盐))',
   salt                 varchar(32) comment '盐',
   nickname             varchar(20) comment '昵称',
   sex                  tinyint(4) default 0 comment '性别(0:未知,1:男,2:女)',
   expired              tinyint(4) default 0 comment '过期字段，1为过期，若过期不可用',
   disabled             tinyint(4) default 0 comment '为不可用字段段，1为不可用，需要修改密码',
...
(
   user_log_id          varchar(32) not null  comment '编号',
...
(
   user_oauth_id        varchar(32) not null  comment '编号',
   user_id              varchar(32) not null comment '帐号编号',
   oauth_id             varchar(32) not null comment '认证方式编号',
